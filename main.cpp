...
public:
  ...
  void display() {
    if (size == 0) {
      ...
      for (int i = 0; i < size; i++) {
        cout << "Floor " << details[i].floorNumber << ", Position "
             << details[i].position << "\n";
      }
    }
  }
...
public:
  OverflowWaitlist() { head = tail = nullptr; }
...
public:
  ...
  void push(int carNumber, int floorNumber, ParkingDetails &details,
    ...
    if (isFull()) {
      ...
      cout << "Car " << carNumber << " parked on Floor " << floorNumber
           << ", Position " << top + 1 << ".\n";
    }
  }
